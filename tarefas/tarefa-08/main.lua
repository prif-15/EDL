...
-- tarefa - 08 - Closure do objeto inimigo

function novoInimigo (x,y,estadoCaminho, vida, velocidade, imagem)
    local objetoInimigo
	
	objetoInimigo = {
	
					setX = function (coordX)
							x = x + coordX
						return x
					end,
												
					setY = function (coordY)
							y = y + coordY
							return y
							end,
					...
					setEstadoCaminho = function (numEstadoCaminho)
											estadoCaminho = numEstadoCaminho
											return estadoCaminho
										  end,
										
					setVida = function (qtdDiminuirVida)
								vida = vida - qtdDiminuirVida
					   		  return vida
							 end,
										  
					setEstadoCaminho = function (aumentoEstadoCaminho)
										 estadoCaminho = estadoCaminho + aumentoEstadoCaminho
										 return estadoCaminho
									   end,
...
function atualizarInimigos(dt)
	...
	if tempoCriacaoInimigos >= constante_intervaloDeAparicaoDeInimigos then
		...
		enemy = novoInimigo(constante_xInicialInimigo,constante_yInicialInimigo,1,constante_vidaInimigoAzulTam1,constante_velocidadeInimigoAzulTam1,imgInimigoAzulTam1)
		table.insert(inimigos, enemy)		
	end
	
	for index = table.getn(inimigos), 1, -1 do
		inimigo = inimigos[index]
		local vidaInimigo = inimigo.getVida() 
		local numEstadoCaminhoInimigo = inimigo.getEstadoCaminho()
		
		if vidaInimigo <= 0 or numEstadoCaminhoInimigo == 6 then
			if numEstadoCaminhoInimigo == 6 then --pq o quantidade de estados sao 5
				cursor.perdeu = true
			end
			-- tarefa - 07 	--> desalocacao de inimigo: tempo de vida de inimigo acaba abaixo
			table.remove(inimigos, index) 
		else
			-- tarefa - 08 -Co-rotina que realiza a movimentacao do inimigo
			coroutine.resume(inimigo.co_rotina, dt) 
		end
	end
...
function checaColisoes(obj1, obj2)	--obj1 é o objeto inimigo
	w1 = obj1.getImagem():getWidth()
	h1 = obj1.getImagem():getHeight()
	w2 = obj2.imagem:getWidth()
	h2 = obj2.imagem:getHeight()
	-- primeiro considerando da perspectiva horizontal
	return ( obj1.getX() - w1/2 <= obj2.x + w2/2 and obj1.getX() + w1/2 >= obj2.x - w2/2 and obj2.y >= obj1.getY() - h1 / 2 and obj2.y <= obj1.getY() + h1/2) or
			-- depois da perspectiva vertical
			( obj1.getY() - h1/2 <= obj2.y + h2/2 and obj1.getY() + h1/2 >= obj2.y - h2/2 and obj2.x >= obj1.getX() - w1/2 and obj2.x <= obj1.getX() + w1/2)
end
...
function drawInimigoVida(inimigo)
	...
	acimaDaCabeca = inimigo.getY() - inimigo.getImagem():getHeight()*0.7  --posicao y da barra de vida do inimigo (acima dele)
	
	pontoDeInicioBarra = inimigo.getX() - (constante_vidaInimigoAzulTam1*constante_multiplicadoraBarras)*1/2  --posicao x da barra de vida do inimigo (acima dele)
	...
	love.graphics.line(pontoDeInicioBarra, acimaDaCabeca, pontoDeInicioBarra + inimigo.getVida()*constante_multiplicadoraBarras, acimaDaCabeca)
	
	love.graphics.setColor(0, 0, 0)
	love.graphics.print(inimigo.getVida(), inimigo.getX() - 5, acimaDaCabeca - 5, 0, 1, 1)
...
function love.draw()
	...
	for index, inimigo in ipairs(inimigos) do
		love.graphics.draw(inimigo.getImagem(), inimigo.getX(), inimigo.getY() , 0, 1, 1, inimigo.getImagem():getWidth()/2, inimigo.getImagem():getHeight()/2)
